		...
		if ( iScrollTop < this.s.redrawTop || iScrollTop > this.s.redrawBottom ) {
			...
			if ( iTopRow != this.s.dt._iDisplayStart ) {
				 ...
				 */
                                ...
                                //Adding correlated alarms' length so those are also considered in the length
                                var iDisplayLength = this.s.dt._iDisplayLength + $(".trCorrelate").length;
				if ( this.s.dt.oFeatures.bServerSide && this.s.dt.aoData.length === iDisplayLength) {
					clearTimeout( this.s.drawTO );
					this.s.drawTO = setTimeout( draw, this.s.serverWait );
				}
                                else if( this.s.tableTop > 0 && this.s.dt.aoData.length < iDisplayLength){
                                        clearTimeout( this.s.drawTO );
					this.s.drawTO = setTimeout( draw, this.s.serverWait );
                                }
                                else if(this.s.dt.aoData.length < iDisplayLength){
                                    console.log("Table end do not fetch data");
                                }
				else {
                                        draw();
				}
...
//		}
	...
	"_fnCalcRowHeight": function ()
	{
		...
		this.s.heights.row = Math.floor( $('tr', tbody).eq(1).outerHeight()/2);
		container.remove();
	},
	...
	"_fnInfo": function ()
	{
		...
		var
			...
			iPossibleEnd = Math.ceil( this.fnPixelsToRow(iScrollTop+this.s.heights.viewport, false, this.s.ani) ),
                        iTotal = ( (this.dom.scroller.scrollTop === 0 && this.s.dt.aoData.length < iPossibleEnd )? iTotal = this.s.dt.aoData.length : dt.fnRecordsDisplay()),
			iEnd = iTotal < iPossibleEnd ? iTotal : iPossibleEnd,
			sStart = dt.fnFormatNumber( (iEnd !==0 ? iStart : iEnd) ),
			sEnd = dt.fnFormatNumber( (iPossibleEnd > iEnd ? iEnd : iEnd -1)),
